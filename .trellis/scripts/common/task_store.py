...
def cmd_archive(args: argparse.Namespace) -> int:
    ...
    if "archived_to" in result:
        ...
        # Post-archive metadata hygiene: rewrite stale JSONL references that
        # point back at the old active-task path, and warn when the archived
        # PRD still has unchecked acceptance boxes.
        rewritten_jsonl = _rewrite_archive_references(
            repo_root, tasks_dir, dir_name, archive_dest
        )
        _warn_unchecked_prd_boxes(archive_dest)

        # Auto-commit unless --no-commit
        if not getattr(args, "no_commit", False):
            if not _auto_commit_archive(dir_name, repo_root, modified_children, rewritten_jsonl):
                print(
                    colored(
...
def _rewrite_archive_references(
    repo_root: Path,
    tasks_dir: Path,
    dir_name: str,
    archive_dest: Path,
) -> list[str]:
    """Rewrites JSONL context entries that point at the freshly archived task.

    After ``task.py archive`` moves ``.trellis/tasks/<dir_name>/`` to
    ``.trellis/tasks/archive/<YYYY-MM>/<dir_name>/``, any ``implement.jsonl``
    / ``check.jsonl`` entry whose ``file`` starts with the old active-task
    path is stale. Rewrites each such entry (in every jsonl under the tasks
    tree, including the moved task's own files) to the archive location.

    Returns the repo-relative paths of the rewritten jsonl files so the
    caller can stage them in the archive commit.
    """
    old_prefix = f"{DIR_WORKFLOW}/{DIR_TASKS}/{dir_name}/"
    new_prefix = _repo_relative_path(archive_dest, repo_root) + "/"
    if old_prefix == new_prefix or not new_prefix.startswith(f"{DIR_WORKFLOW}/{DIR_TASKS}/"):
        return []

    rewritten: list[str] = []
    for jsonl_file in sorted(tasks_dir.rglob("*.jsonl")):
        ...
        for line in lines:
            ...
            if not file_path or not file_path.startswith(old_prefix):
                out.append(line)
                continue
            rest = file_path[len(old_prefix):]
            data["file"] = new_prefix + rest
            out.append(json.dumps(data, ensure_ascii=False))
            changed = True
            print(colored(
                f"  Rewrote reference: {file_path} -> {data['file']}", Colors.YELLOW
            ))
        if changed:
            jsonl_file.write_text("\n".join(out) + "\n", encoding="utf-8")
            rewritten.append(_repo_relative_path(jsonl_file, repo_root))
    return rewritten


def _warn_unchecked_prd_boxes(archive_dest: Path) -> None:
    """Warn when the archived task's PRD still has unchecked acceptance boxes.

    Doesn't block archiving (a task may legitimately carry follow-up items),
    but surfaces the gap so metadata doesn't silently claim completion.
    """
    prd = archive_dest / "prd.md"
    if not prd.is_file():
        return
    unchecked = [
        line for line in prd.read_text(encoding="utf-8").splitlines()
        if re.match(r"^-\s+\[ \]", line)
    ]
    if unchecked:
        print(colored(
            f"[WARN] {archive_dest.name}/prd.md has {len(unchecked)} unchecked "
            f"acceptance box(es); record evidence (validation-evidence.md / "
            f"implement.md) or confirm before declaring the task done.",
            Colors.YELLOW,
        ))
...
def _auto_commit_archive(
    ...
    if extra_paths:
        # Include jsonl files (possibly in OTHER tasks) whose entries were
        # rewritten to the archive location by this archive command.
        paths = list(dict.fromkeys([*paths, *extra_paths]))
    if not paths:
