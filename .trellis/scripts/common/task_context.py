...
def _resolve_context_path(repo_root: Path, file_path: str) -> Path | None:
    """Resolve a JSONL context entry to a path that actually exists.

    Tries the literal ``repo_root / file_path`` first; when that is missing
    and the entry points into the tasks tree (``.trellis/tasks/<name>/...``),
    falls back to the task archive — the referenced task may have been
    archived and moved to ``.trellis/tasks/archive/<YYYY-MM>/<name>/...``
    after the entry was written.

    Returns ``None`` when nothing resolvable is found (caller reports the
    broken entry) or when the archive fallback is ambiguous (same task dir
    name archived in multiple months).
    """
    direct = repo_root / file_path
    if direct.exists():
        return direct

    m = re.match(r"^\.trellis/tasks/([^/]+)/(.+)$", file_path)
    if not m:
        return None
    task_name, rest = m.group(1), m.group(2)
    hits = sorted((repo_root / ".trellis/tasks").glob(f"archive/*/{task_name}/{rest}"))
    if len(hits) != 1:
        return None
    return hits[0]
...
def _validate_jsonl(jsonl_file: Path, repo_root: Path) -> int:
    ...
    for line in jsonl_file.read_text(encoding="utf-8").splitlines():
        ...
        real_entries += 1
        resolved = _resolve_context_path(repo_root, file_path)
        if resolved is None:
            print(f"  {colored(f'{file_name}:{line_num}: File not found: {file_path}', Colors.RED)}")
            errors += 1
            continue

        if resolved != repo_root / file_path:
            # Archived reference — resolved through the archive fallback.
            rel = _repo_relative(resolved, repo_root)
            print(f"  {colored(f'{file_name}:{line_num}: resolved via archive: {rel}', Colors.YELLOW)}")
            continue
