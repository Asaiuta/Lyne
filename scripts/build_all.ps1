...
$pkgConfigPaths = @()
$pathAdditions = @()
$msys2Bin = Join-Path $env:USERPROFILE 'scoop\apps\msys2\current\mingw64\bin'
$msys2PkgConfig = Join-Path $env:USERPROFILE 'scoop\apps\msys2\current\mingw64\lib\pkgconfig'
$vcpkgRootCandidates = @(
    $env:VCPKG_ROOT,
    'H:\VCP\vcpkg'
) | Where-Object { $_ }
...
foreach ($candidate in $vcpkgRootCandidates) {
    ...
    $staticMd = Join-Path $candidate 'installed\x64-windows-static-md'
    $static = Join-Path $candidate 'installed\x64-windows-static'
    if (Test-Path $staticMd) {
...
if ($VcpkgBase) {
    Write-Host ">>> Using vcpkg from: $VcpkgBase" -ForegroundColor Cyan
    $pkgconfTools = Join-Path $VcpkgBase 'tools\pkgconf'
    $vcpkgPkgConfig = Join-Path $VcpkgBase 'lib\pkgconfig'
    if (Test-Path $pkgconfTools) { $pathAdditions += $pkgconfTools }
    if (Test-Path $vcpkgPkgConfig) { $pkgConfigPaths += $vcpkgPkgConfig }
} else {
    Write-Host '>>> Info: no compatible vcpkg soxr install detected; falling back to MSYS2 pkg-config.' -ForegroundColor Yellow
}

if (Test-Path $msys2Bin) { $pathAdditions += $msys2Bin }
if (Test-Path $msys2PkgConfig) { $pkgConfigPaths += $msys2PkgConfig }

if ($pathAdditions.Count -gt 0) {
    $env:PATH = (($pathAdditions + @($env:PATH)) | Where-Object { $_ }) -join ';'
}
if ($pkgConfigPaths.Count -gt 0) {
    $env:PKG_CONFIG_PATH = ($pkgConfigPaths | Select-Object -Unique) -join ';'
}
