...
mod win {
  ...
  fn create_controls(
    ...
    controls
      .attach(move |event| {
        ...
        if let Err(error) = app_handle.emit_to(MAIN_WINDOW_LABEL, SMTC_CONTROL_EVENT, payload) {
  ...
  mod tests {
    ...
    fn media_keys_map_to_frontend_actions() {
      assert_eq!(control_action(&MediaControlEvent::Play), Some(("play", None)));
      assert_eq!(control_action(&MediaControlEvent::Pause), Some(("pause", None)));
      assert_eq!(control_action(&MediaControlEvent::Toggle), Some(("toggle", None)));
      assert_eq!(control_action(&MediaControlEvent::Next), Some(("next", None)));
      assert_eq!(
        control_action(&MediaControlEvent::Previous),
        Some(("previous", None))
      );
      assert_eq!(control_action(&MediaControlEvent::Stop), Some(("pause", None)));
    }

    #[test]
    fn set_position_maps_to_absolute_seek_seconds() {
      let event = MediaControlEvent::SetPosition(MediaPosition(Duration::from_millis(93_500)));
      assert_eq!(control_action(&event), Some(("seek", Some(93.5))));
    }

    #[test]
    fn events_without_an_app_action_are_dropped() {
      assert_eq!(control_action(&MediaControlEvent::Seek(SeekDirection::Forward)), None);
      assert_eq!(
    ...
    fn duration_guard_rejects_non_finite_and_negative_values() {
      assert_eq!(duration_from_secs(12.25), Some(Duration::from_secs_f64(12.25)));
    ...
    fn metadata_maps_borrowed_fields_and_duration() {
      ...
      assert_eq!(mapped.cover_url, Some("http://127.0.0.1:63790/cover?token=abc"));
