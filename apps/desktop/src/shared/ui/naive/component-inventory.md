...
| `NaiveMenu` | `NMenu` menu option rendering | generic handwritten menu facade with roving focus, arrow-key, Home/End navigation; `SettingsCategoryNav` is the first consumer | source-backed consumer |
...
| `NaiveDropdown` | `NDropdown` | package-level facade with startup-light public proxy plus lazy Kobalte `DropdownMenu` implementation; trigger-anchored + virtual (x/y/show) modes and recursive cascade submenus | source-backed trigger-anchored + virtual + cascade |
...
| `NaiveSwitch` | `NSwitch` | source-backed NaiveUI 2.43.2 state/class facade with lightweight fallback plus lazy Kobalte `Switch`; keeps pressed/loading hooks, `.n-base-loading`, and non-boolean checked/unchecked value round trips while preserving boolean settings callers | source-backed value-ready |
...
| `NaiveColorPicker` | `NColorPicker` | compact controlled color-well facade over the browser color chooser; preserves `.n-color-picker` hooks and preview/complete callbacks; consumed by AppearanceAdvancedPanels | source-backed consumer |
...
| `NaiveSlider` | `NSlider` | source-backed NaiveUI 2.43.2 single-thumb slider facade with startup-light public proxy plus lazy Kobalte `Slider`; emits `.n-slider*` rail, fill, handle, tooltip, marks, vertical, disabled, and with-mark hooks | source-backed volume + EQ migrated |
...
| long-tail feature controls | `NDynamicTags` / `NTree` / `NDataTable` | no package facades yet; implement only from consumer-backed feature tasks so tag editing, tree navigation, and reusable data-table contracts do not ship as dead exports | deferred |
...
| `NSlider` | 6 | `NaiveSlider` source-backed/Kobalte facade; PlayerVolumePopover vertical volume and settings EQ bands are migrated, while settings RangeInput, progress sliders, and long-tail range behavior stay deferred |
...
| `NMenu` | 3 | `NaiveMenu` generic facade; Sidebar and SettingsCategoryNav keep shell-specific wrappers/classes |
...
| `NColorPicker` | 2 | `NaiveColorPicker` consumer facade used by AppearanceAdvancedPanels; native browser chooser remains the platform interaction |
...
- 2026-07-18: Completed the active-control migration pass against the current SPlayer surfaces. Online-only search and account controls now disappear with `useOnlineService`; search/form controls use the package facades; `NaiveMenu` owns settings-category keyboard navigation; `NaiveColorPicker` owns appearance swatches; `NaiveTabs` owns `SegmentedTabs`; `NaiveDropdown` owns the virtual-anchor `ContextMenu` and recursive cascade path; EQ controls use `NaiveSlider`. The remaining native controls are isolated to the unreferenced `QueuePage.tsx` fixture and to contracts intentionally kept local (transport, window, modal, card/list and load-more actions).
...
- 2026-05-26: Migrated `SongWikiPage` title/section headings/loading state/skeleton blocks to `NaiveH2`, `NaiveH3`, `NaiveSpin`, and `NaiveSkeleton`. Kept existing page classes; `SongWikiPage` now imports its route-owned `song-wiki.css` stylesheet at the lazy page root.
