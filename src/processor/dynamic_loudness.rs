...
impl BiquadFilter {
    ...
    fn set_gain_db(&mut self, gain_db: f64) {
        self.coeffs = match self.filter_type {
            FilterType::Peaking => {
                Self::calc_peaking_coeffs(self.freq, gain_db, self.q, self.sample_rate)
            }
            FilterType::LowShelf => {
                Self::calc_low_shelf_coeffs(self.freq, gain_db, self.sample_rate)
            }
            FilterType::HighShelf => {
                Self::calc_high_shelf_coeffs(self.freq, gain_db, self.sample_rate)
            }
        };
    }
    ...
    fn process(&mut self, x: f64) -> f64 {
        let y = scrub_denormal(self.coeffs.b0 * x + self.state.z1);
        self.state.z1 = scrub_denormal(self.coeffs.b1 * x - self.coeffs.a1 * y + self.state.z2);
        self.state.z2 = scrub_denormal(self.coeffs.b2 * x - self.coeffs.a2 * y);
        y
    }
    ...
    fn set_sample_rate(&mut self, sample_rate: f64) {
        if (self.sample_rate - sample_rate).abs() > 1.0 {
            ...
            self.coeffs = match self.filter_type {
                FilterType::Peaking => {
                    Self::calc_peaking_coeffs(self.freq, 0.0, self.q, sample_rate)
                }
...
mod tests {
    ...
    #[test]
    fn test_biquad_scrubs_denormals() {
        let mut filter = BiquadFilter::peaking(1000.0, 0.0, 1.0, 44100.0);
        filter.state.z1 = 1.0e-40;
        filter.state.z2 = -1.0e-40;
