...
impl AppConfig {
    pub fn load() -> Self {
        ...
        let resample_quality = match env::var("AUDIO_RESAMPLE_QUALITY").unwrap_or_default().as_str() {
        ...
        let cache_dir = env::var("AUDIO_CACHE_DIR")
            .ok()
            .map(PathBuf::from);
        ...
        let loudness = LoudnessConfig {
            ...
            smoothing_time_ms: env::var("AUDIO_LOUDNESS_SMOOTHING_MS")
                ...
                .unwrap_or(200.0)
                .clamp(10.0, 2000.0),
            mode: match env::var("AUDIO_NORMALIZATION_MODE").unwrap_or_default().to_lowercase().as_str() {
        ...
        let phase_response = match env::var("AUDIO_PHASE_RESPONSE").unwrap_or_default().to_lowercase().as_str() {
        ...
        let saturation = SaturationConfig {
            sat_type: match env::var("AUDIO_SATURATION_TYPE").unwrap_or_default().to_lowercase().as_str() {
