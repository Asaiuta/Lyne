...
pub(super) enum ProxyRouteGroup {
    Auth,
    Search,
    Catalog,
    Playlist,
...
#[cfg(test)]
pub(super) fn proxy_method_registry() -> &'static [(ProxyRouteGroup, &'static [&'static str])] {
    PROXY_METHOD_REGISTRY
}
...
#[cfg(test)]
pub(super) fn proxy_handler_method_names() -> Vec<&'static str> {
...
pub(super) fn login_qr_key_payload() -> serde_json::Value {
    json!({ "type": 3 })
}

pub(super) fn login_qr_check_payload(query: &Query) -> serde_json::Value {
    json!({
