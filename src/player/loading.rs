...
use super::buffer_budget::{
    ...
    published_decoded_samples, record_budget_rejection, reserve_decoded_buffer_bytes,
    reserve_decoded_buffer_capacity, DecodedBufferKind,
};
use super::cache::{
...
pub(super) fn decode_file_internal(
    ...
    if let Some(ref cp) = cache_path {
        if cp.exists() {
            let cache_reservation = if let Ok(metadata) = std::fs::metadata(cp) {
                ...
                let cache_bytes = usize::try_from(metadata.len())
                    .map_err(|_| "resample cache size exceeds addressable memory".to_string())?;
                Some(record_budget_rejection(
                    shared_state,
                    reserve_decoded_buffer_bytes(DecodedBufferKind::ResampleCache, cache_bytes),
                )?)
            } else {
                None
            };
            if let Some(cached_samples) = cache_reservation
                .as_ref()
                .and_then(|_| load_cache_with_header(cp, final_target_sr, channels as u32))
            {
                record_budget_rejection(
                    shared_state,
                ...
                let reservation = cache_reservation.ok_or_else(|| {
                    "resample cache metadata was unavailable before allocation".to_string()
                })?;
    ...
    let reservation = record_budget_rejection(
        ...
        reserve_decoded_buffer_bytes(
            ...
            output_sample_capacity
                ...
                .ok_or_else(|| "decoded buffer reservation overflowed".to_string())?,
        ),
    )?;
