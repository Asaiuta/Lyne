...
mod tests {
    ...
    fn published_window(slot_count: usize) -> (PcmWindowReader, usize) {
        ...
        let parts =
            PcmWindow::create(geometry, 5, 100, DecodedMemoryOwner::ActiveWindow).expect("window");
    ...
    fn cache_clones_only_on_generation_change_and_retires_displaced_window() {
        ...
        let first = PcmWindow::create(geometry, 1, 0, DecodedMemoryOwner::ActiveWindow)
            .expect("first window");
        let second = PcmWindow::create(geometry, 2, 0, DecodedMemoryOwner::ActiveWindow)
